...
python -m img_recongition_module.ocr_video /path/to/video.mp4 [--output output.json] [--frame_interval 3] [--max_frames 15] [--api_key your_api_key]
...
# 添加一个保存OCR结果的辅助函数
def save_ocr_result(result, input_path, output_dir="./data/output"):
    """
    保存OCR结果到JSON文件
    ...
    # 构建输出文件路径
    output_path = os.path.join(output_dir, f"{input_name}.json")
...
def run_url_image_test(ocr):
    ...
    # 保存结果
    output_path = save_ocr_result(result, "url_image")
...
def run_local_image_test(ocr, image_path=None):
    ...
    if image_path is None:
        image_path = r".\data\input\test_01.jpg"
    ...
    # 保存结果
    output_path = save_ocr_result(result, image_path)
    print(f"结果已保存到: {output_path}")
    print()
...
def run_unified_interface_test(ocr, url_image, local_image):
    ...
    print(f"识别结果: {result}")
    # 保存结果
    output_path = save_ocr_result(result, "unified_url_image")
    print(f"结果已保存到: {output_path}")
    print()
    ...
    print(f"识别结果: {result}")
    # 保存结果
    output_path = save_ocr_result(result, local_image)
    print(f"结果已保存到: {output_path}")
    print()
...
def run_multiple_images_test(ocr, local_image):
    ...
    print(f"识别结果: {result}")
    # 保存结果
    output_path = save_ocr_result(result, "multiple_url_images")
    print(f"结果已保存到: {output_path}")
    print()
    ...
    print(f"识别结果: {result}")
    # 保存结果
    output_path = save_ocr_result(result, "mixed_images")
    print(f"结果已保存到: {output_path}")
    print()
...
def run_chat_single_test(ocr, image_path=None):
    ...
    try:
        ...
        # 保存结果
        output_path = save_ocr_result(chat_data, image_path)
        print(f"结果已保存到: {output_path}")
...
def run_chat_multiple_test(ocr):
    ...
    try:
        ...
        # 保存结果
        output_path = save_ocr_result(chat_data, "test_02")
        print(f"结果已保存到: {output_path}")
    except Exception as e:
        print(f"解析结果失败: {str(e)}")
        print(f"原始结果: {result}")
        # 保存原始结果
        output_path = save_ocr_result(result, "test_02")
        print(f"原始结果已保存到: {output_path}")
...
def run_chat_video_test(ocr, video_path=None):
    ...
    try:
        ...
        # 保存结果
        output_path = save_ocr_result(chat_data, video_path)
        print(f"结果已保存到: {output_path}")
    except Exception as e:
        print(f"解析结果失败: {str(e)}")
        print(f"原始结果: {result}")
        # 保存原始结果
        output_path = save_ocr_result(result, video_path)
        print(f"原始结果已保存到: {output_path}")
...
def process_video(video_path=None, frame_interval=1, max_frames=30, api_key=None, save_json=None):
    """
    便捷函数，用于直接处理视频文件
    ...
    try:
        ...
        if save_json:
            ...
            # 如果未指定保存路径，则使用默认路径保存
            save_ocr_result(chat_data, video_path)
...
def ocr_video_cli():
    ...
    parser.add_argument("--frame_interval", "-i", type=int, default=1, help="视频帧间隔，每隔多少帧提取一次，默认为1")
    parser.add_argument("--max_frames", "-m", type=int, default=30, help="最大提取帧数，默认为30")
    ...
    try:
        ...
        if args.output:
            ...
            # 使用默认输出路径
            output_path = save_ocr_result({}, args.video_path)
            # 删除刚刚创建的空文件，因为下面会实际写入内容
...
def main():
    ...
    parser.add_argument("--frame_interval", type=int, default=1, help="视频帧间隔，每隔多少帧提取一次，默认为1")
    parser.add_argument("--max_frames", type=int, default=30, help="最大提取帧数，默认为30")
    ...
    try:
        ...
        if args.quick_process:
            ...
            if args.save_json:
                ...
                # 自动保存到默认路径
                output_path = save_ocr_result(chat_data, args.quick_process)
                print(f"\n结果已保存到: {output_path}")
